...
export interface GraphVM {
  nodes: Array<ForceNode & { type: string }>;
  edges: ForceEdge[];
  stats: GraphResponse['graph']['stats'];
}

export function toGraphVM(raw: GraphResponse, width: number, height: number): GraphVM {
  const nodes: Array<ForceNode & { type: string }> = raw.graph.nodes.map((n, i) => {
    const angle = (2 * Math.PI * i) / Math.max(1, raw.graph.nodes.length);
    return { id: n.id, type: n.type, x: width / 2 + 140 * Math.cos(angle), y: height / 2 + 140 * Math.sin(angle), vx: 0, vy: 0 };
  });
